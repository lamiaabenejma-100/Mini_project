...
with n1:
    if st.button(":material/search:", key="nav1"):
        st.session_state.view_mode = "🎯"
with n2:
    if st.button(":material/auto_awesome:", key="nav2"):
        st.session_state.view_mode = "🧩"
with n3:
    if st.button(":material/database:", key="nav3"):
        st.session_state.view_mode = "📂"
...
with content_col:
    ...
    if st.session_state.view_mode == "🎯":
        ...
        with cols[1]:
            run_btn = st.button(":material/send:", key="run_match")

        if run_btn:
            db = [
                c for c in st.session_state.candidate_memory
                if target_job.lower() in str(c.get("applying_for", "")).lower()
            ]
            ...
            if not db:
                ...
                with st.spinner("Analyzing..."):
                    ...
                    count = len(db)
                    task_type = "Rank the candidates" if count > 1 else "Evaluate the candidate"

                    match_prompt = f"""<|begin_of_text|><|start_header_id|>system<|end_header_id|>
You are a professional HR assistant.
STRICT DATA LIMIT: {count} candidate(s).
CONTEXT:
{db_context}
<|eot_id|>
<|start_header_id|>user<|end_header_id|>
{task_type} for role "{target_job}" based on criteria:
{criteria}
<|eot_id|>
...
"""
                    ...
                    response = requests.post(
                        API_URL,
                        json={"task": "analyze", "text": match_prompt}
                    )

                    st.session_state.last_res = response.json().get("result", "")

        if st.session_state.last_res:
            st.markdown(
                f'<div class="response-box">{st.session_state.last_res}</div>',
    ...
    # ===== EXTRACTOR 🧩 =====
        ...
        cols = st.columns([0.93, 0.07])
        with cols[1]:
            extract_btn = st.button(":material/add_task:", key="run_extract")

        if extract_btn:
            if not raw_text.strip():
                st.toast("Please provide text.", icon="⚠️")
            else:
                with st.spinner("Processing..."):
                    response = requests.post(
                        API_URL,
                        json={"task": "extract", "text": raw_text}
                    )
                    ...
                    if "error" in data:
                        st.error(data["error"])
                    elif run_integrity_pipeline(data):
    ...
    # ===== DATABASE 📂 =====
        ...
        if st.session_state.candidate_memory:
            ...
            st.markdown(
                "<div style='text-align:center; color:#444; margin-top:80px;'>Intelligence database is empty.</div>",
                unsafe_allow_html=True
            )
